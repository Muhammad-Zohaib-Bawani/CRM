...
export default function TicketModal({ mode, ticket, onClose }) {
    ...
    isCreate
      ...
      : {
          ...
          dueDate: ticket.dueDate
            ? new Date(ticket.dueDate + "T00:00:00")
            : null,
        },
  );
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      if (r.status === "fulfilled") {
        ...
        const url =
          typeof raw === "string"
            ? raw
            : raw?.imageUrl || raw?.url || raw?.fileUrl || null;
        if (!url) {
        ...
        if (!isCreate && ticket?.id) {
          try {
            const saved = await addAttachmentApi(ticket.id, {
              fileName: f.name,
              fileUrl: url,
              fileSize: f.size,
            });
  ...
  const handleAssign = async (opt) => {
    setAssigning(true);
    try {
      await assignTicket(ticket.id, opt?.value || null);
    } finally {
      setAssigning(false);
    }
  };
  ...
  const handleAddComment = async () => {
    ...
    setCommenting(true);
    try {
      const newComment = await addComment(ticket.id, {
        authorId: user.id,
  ...
  const author = t.createdBy
    ? users.find((u) => u.id === t.createdBy) ||
      (t.createdBy === user.id ? user : null)
    : null;
  const assignee = t.assignedTo
    ? users.find((u) => u.id === t.assignedTo) ||
      (t.assignedTo === user.id ? user : null)
    : null;
  const isOverdue =
    !isCreate &&
    t.dueDate &&
          ...
          <div className="modal-body">
            {!isCreate && (
              ...
              >
                ...
                <span
                  className={`badge badge-pri-${(t.priority || "medium").toLowerCase()}`}
                >
                  {t.priority}
                </span>
                <span
            ...
            {isCreate || canEditAll ? (
              <>
                ...
                <div className="field-row">
                  <div className="field">
                    ...
                    <Select
                      options={typeOpts}
                      value={
                        typeOpts.find((o) => o.value === form.type) || null
                      }
                      onChange={(opt) =>
                        update("type", opt?.value || ticketTypes[0])
                      }
                      styles={rsStyles}
                  ...
                  <div className="field">
                    ...
                    <Select
                      ...
                      value={
                        PRIORITY_OPTS.find((o) => o.value === form.priority) ||
                        null
                      }
                      onChange={(opt) =>
                        update("priority", opt?.value || "Medium")
                      }
                      styles={rsStyles}
                      menuPortalTarget={document.body}
                      isSearchable={false}
                  ...
                  {(canAssign || isCreate) && (
                    <div className="field">
                      ...
                      <Select
                        ...
                        value={
                          agentOpts.find(
                            (o) => o.value === (form.assignedTo || ""),
                          ) || agentOpts[0]
                        }
                        onChange={(opt) =>
                          update("assignedTo", opt?.value || "")
                        }
                        styles={rsStyles}
                        menuPortalTarget={document.body}
                      />
                ...
                <div className="field">
                  ...
                  >
                    ...
                    {uploadingCount > 0 ? (
                      <span
                        style={{ fontSize: 11, color: "var(--brand-deep)" }}
                      >
                        <i
                  ...
                  {(form.attachments || []).length > 0 && (
                    <div className="attachment-list">
                      {form.attachments.map((a, i) => {
                        ...
                        return (
                          <div
                            className={`attachment-chip${imgSrc ? " has-img" : ""}`}
                            key={i}
                          >
                            {imgSrc ? (
                              <a
              ...
              <>
                ...
                >
                  <InfoRow
                    label="Assignee"
                    value={assignee?.name || t.assignedToName || "Unassigned"}
                  />
                  <InfoRow
                    label="Due date"
                    value={
                ...
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  {[
                    ...
                    <button
                      ...
                      className={`btn btn-sm ${t.status === s.v ? "btn-primary" : "btn-ghost"}`}
                    >
                      {s.l}
                    </button>
                  ))}
            ...
            {!isCreate && canAssign && (
              <div className="field">
                ...
                <Select
                  options={agentOpts}
                  value={
                    agentOpts.find((o) => o.value === (t.assignedTo || "")) ||
                    agentOpts[0]
                  }
                  ...
                  return (
                    <div className="comment" key={c.id}>
                      <div className="avatar">
                        {cAuthor?.initials || c.userName?.[0] || "?"}
                      </div>
                      <div className="body">
                        <div className="head">
                          <strong>
                            {cAuthor?.name || c.userName || "Unknown"}
                          </strong>
                          <span className="time">
...
function activityText(a) {
  ...
  switch (a.activityType) {
    ...
    case "type_changed":
      return `${who} changed type from "${a.oldValue}" to "${a.newValue}"`;
    case "due_date_changed":
      return `${who} changed due date from "${a.oldValue || "none"}" to "${a.newValue}"`;
    case "assigned_to_changed":
      return `${who} reassigned from "${a.oldValue}" to "${a.newValue}"`;
    default:
      return `${who} updated ${a.field || "ticket"}`;
  }
}
      ...
      >
        ...
        {activities.map((a) => (
          ...
          >
            ...
            >
              <i
                className={`fa-solid ${ACTIVITY_ICONS[a.activityType] || "fa-circle-dot"}`}
              />
            </div>
